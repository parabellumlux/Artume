...
4. Debug (breakpoints/step/inspect) — 🟡 **usable** (adapter launches via
   `debug_adapter.py`, breakpoint/step/continue/evaluate work, and stepping now
   announces the paused position + locals by voice); not yet battery-tested
   against a real multi-frame program.
...
**Audio IDE (PLAN_AUDIO_IDE §12, 10 criteria):** 1 open project 🟢 · 2 skim structure 🟢 (spoken "skim code/table of contents, list functions/classes" → concise TOC: "1 function, 1 class in 100 lines. helper, line 3 …", bounded at 15 entries) · 3 navigate 🟢 (spoken: next/prev function · class · method · line, jump/go-to-line N, top/end of file — each move reads the landed line) · 4 read code 🟢 · 5 show errors 🟢 (voice path + py_compile/pyflakes) · 6 edit-in-place 🟢 (spoken "insert line/after line N …", "replace/change line N with …", "delete lines A to B", "delete current line" → edits the open file, re-anchors the daemon cursor, reads the changed line back; routed any-mode via `ide_action edit:*`) · 7 debug 🟢 (DAP adapter live; every step now announces the paused position via `stackTrace` — "Paused at line N in f, file x.py"; speak "where is the debugger", "pause execution", and "list locals" reads the top-frame Locals scope) · 8 inspect variables 🟢 (evaluate `expr`, "list locals"/"read locals", stackTrace-position readout; watch via DAP scopes) · 9 git 🟢 · 10 run tests 🟢. **10/10 fully met.**
