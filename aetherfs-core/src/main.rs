...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for target in &watch_targets {
        ...
        println!(
            "AetherFS Watcher: Monitoring directory: {}",
            target.display()
        );
    }
    ...
    tokio::spawn(async move {
        ...
        let mut last_indexed: std::collections::HashMap<std::path::PathBuf, std::time::Instant> =
        ...
        while let Some(evt) = watcher_rx.recv().await {
            match evt {
                WatcherEvent::Update(path) => {
                    ...
                    if path.is_file() {
                        if let Ok(metadata) = std::fs::metadata(&path) {
                            ...
                            if let Ok(candidates) = index_mgr_proc
                                .sqlite()
                                .find_files_by_size(size, &path.to_string_lossy())
                            {
                                if !candidates.is_empty() {
                                    if let Ok(sh) = dedup::DedupPipeline::calculate_sparse_hash(
                                        &path,
                                        size as u64,
                                    ) {
                                        ...
                                        for cand in candidates {
                                            if cand.sparse_hash.as_ref() == Some(&sh_str) {
                                                if let (Ok(fh), Ok(cand_fh_bytes)) = (
                                                    dedup::DedupPipeline::calculate_full_hash(
                                                        &path,
                                                    ),
                                                    dedup::DedupPipeline::calculate_full_hash(
                                                        Path::new(&cand.path),
                            ...
                            if let Err(e) = index_mgr_proc
                                .index_file(crate::index::FileMeta {
                                    path: &path,
                                    size_bytes: size,
                            ...
                            {
                                eprintln!(
                                    "AetherFS Indexer: Failed to index file {}: {}",
                                    path_display, e
                                );
                            } else {
                                println!(
                                    "AetherFS Indexer: Automatically indexed {}",
                                    path_display
                                );
                            }
                ...
                WatcherEvent::Remove(path) => {
                    ...
                    println!(
                        "AetherFS Watcher: Detected file deletion: {}",
                        path.display()
                    );
                    if let Err(e) = index_mgr_proc.delete_file(&path.to_string_lossy()).await {
                        eprintln!(
                            "AetherFS Indexer: Failed to remove {} from index: {}",
                            path.display(),
                            e
                        );
                    } else {
